...
| 2026-04-17 | MC-250 — COMPLETE | `--destination` + `--max-commute` filter added to find_deals.py. Nominatim geocoding (no API key needed), ORS with header auth, Haversine fallback. neighbourhood_coords.py: ~150 Toronto neighbourhood centroids. test_mc250_commute_filter.py: 5/5 pass. Committed 427c900. | 54 LIVE | varies | union station: 35 listings within 30min (Dovercourt Village, Brockton Village, Liberty Village) | MC-249: fixed score_deals missing `return df`. MC-249 10/10 tests pass. |
