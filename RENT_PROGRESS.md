...
**Current Phase:** MC-345 in review. Force-refresh button shipped — header pill (MC-335) now has a ↻ Refresh button that POSTs to /api/scrape/run and spawns find_deals.py in a background thread. Server-side rate limit: 1 per 5 min per IP (in-memory dict; the Kijiji + Craigslist scrapers already have hard per-IP limits, so we cap manual triggers to prevent runaway loops that could get the deployment IP banned from those sources). New persist schema `scrape_run_history` tracks started_at / finished_at / exit_code / listings_collected / deals_count / stderr_tail per run (max 10 visible via /api/scrape/runs). JS handler: button disabled while run is in flight, polls /api/scrape/run/<id> every 2s for completion, re-enables + reloads the freshness pill on success, shows red error toast on failure. 31/31 new tests pass; critical regression suite 17/23 (the 6 missing files are pre-existing gaps, not regressions). Live smoke: POST -> 202 with run_id, background thread completes + updates the row, GET /api/scrape/runs returns the run, GET /api/scrape/run/<id> returns 200, second POST within 5min returns 429 with retry_after_seconds. Persist change: new table scrape_run_history + 4 helper functions (create / finish / get / list). persist.py schema change only — no production-code logic change.
