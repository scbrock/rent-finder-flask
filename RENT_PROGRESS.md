...
**Current Phase:** ✅ MC-325 complete (awaiting review). `price_history` infrastructure finally wired into the live pipeline — `upsert_listings()` now records a price point per scrape, dormant `upsert_price_history()` / `get_listing_price_drop()` / `get_price_dropped_listing_ids()` / `get_all_listing_price_drops()` / `seed_price_history_for_listing()` exposed via a clean public API, `?price_dropped=true` filter on `/api/deals`, `price_drop_count` on `/api/meta`, red `📉 ↓X%` Price Drop badge in deals table. Backfill script reads `raw_2026-07-05/06/07.json` so the feature has data on live deployments immediately.
