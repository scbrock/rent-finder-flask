...
def deals():
    ...
    MC-319: /api/deals now returns the paginated dict {deals, total, ...}.
    The fixture returns the *list* of deals (the first page only) so existing
    tests can keep iterating over it. Tests that need the total should fetch
...
class TestUrlPersistence:
    ...
    def test_beds_min_in_url(self):
        ...
        assert all(d['beds'] >= 2 for d in resp.get_json()['deals'] if d.get('beds') is not None)
    ...
    def test_shared_link_url_params(self):
        ...
        r1 = client.get(f'/api/deals?{params}').get_json()['deals']
        r2 = client.get(f'/api/deals?{params}').get_json()['deals']
