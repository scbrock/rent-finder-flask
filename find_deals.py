...
# ─── MC-320: Neighbourhood Standardization ─────────────────────────────────────
# Listings scraped with neighbourhood="Toronto" or "" or city-level names
# (Mississauga, Scarborough) need to be mapped to official Toronto neighbourhoods
# so that fair_value grouping by (neighbourhood, beds) actually has signal.
# MC-260 built neighbourhood_lookup.py with 158 official Toronto neighbourhoods
# + direct map for 80+ variants + fuzzy matching. standardize() also falls
# back to title-based extraction when the neighbourhood field is empty.

# Neighborhoods that are too generic to be useful for grouping — try title
# fallback before giving up.
_GENERIC_NEIGHBOURHOODS = {"toronto", "city of toronto", "gt", "gta", "ontario", ""}

def standardize_neighbourhoods(df):
    """
    Apply neighbourhood_lookup.standardize() to each row's neighbourhood field.
    Falls back to title-based extraction and URL-slug extraction when the
    neighbourhood is generic. Adds/updates 'region' column via
    region_map.neighbourhood_to_region.

    Returns the modified df.
    """
    from neighbourhood_lookup import standardize as _standardize
    from neighbourhood_lookup import extract_url_slug
    from region_map import neighbourhood_to_region

    if "neighborhood" not in df.columns:
        df["neighborhood"] = ""

    title_col = df["title"] if "title" in df.columns else None
    link_col = df["link"] if "link" in df.columns else (df["url"] if "url" in df.columns else None)

    new_nbhd = []
    new_region = []
    standardized_count = 0
    title_resolved_count = 0
    slug_resolved_count = 0
    non_toronto_count = 0

    for idx in range(len(df)):
        raw = str(df.at[idx, "neighborhood"]) if df.at[idx, "neighborhood"] else ""
        title = str(title_col.at[idx]) if title_col is not None and title_col.at[idx] else ""
        url = str(link_col.at[idx]) if link_col is not None and link_col.at[idx] else ""
        url_slug = extract_url_slug(url)

        # If the value looks specific (not in _GENERIC_NEIGHBOURHOODS),
        # still try lookup to upgrade it to the official name where possible
        # (e.g. "toronto, on" -> "Bay Street Corridor").
        norm_lower = raw.lower().strip()
        is_generic = norm_lower in _GENERIC_NEIGHBOURHOODS

        if not is_generic:
            result = _standardize(raw, "", url_slug)
            if result:
                df.at[idx, "neighborhood"] = result
                standardized_count += 1
            new_nbhd.append(df.at[idx, "neighborhood"])
            new_region.append(neighbourhood_to_region(df.at[idx, "neighborhood"]))
            continue

        # Generic — try lookup + title + slug fallback
        result = _standardize(raw, title, url_slug)
        # Track which fallback resolved it (for logging)
        title_result = _standardize(raw, title) if title else None
        slug_result = (_standardize(raw, "", url_slug) if url_slug and title_result is None else None) if url_slug else None

        if result is None:
            new_nbhd.append(raw)
            new_region.append(neighbourhood_to_region(raw))
        elif result == "":
            df.at[idx, "neighborhood"] = raw if raw else "Non-Toronto"
            new_nbhd.append(df.at[idx, "neighborhood"])
            new_region.append("")
            non_toronto_count += 1
        else:
            df.at[idx, "neighborhood"] = result
            new_nbhd.append(result)
            new_region.append(neighbourhood_to_region(result))
            # Decide which fallback resolved it (for stats)
            if title_result is not None and title_result == result:
                title_resolved_count += 1
            elif slug_result is not None and slug_result == result:
                slug_resolved_count += 1
            else:
                # Resolved by both title and slug — count under title
                if title:
                    title_resolved_count += 1
                elif url_slug:
                    slug_resolved_count += 1

    print(f"  Standardized {standardized_count} neighbourhoods (specific -> official)")
    print(f"  Resolved {title_resolved_count} generic -> official via title fallback")
    print(f"  Resolved {slug_resolved_count} generic -> official via URL slug fallback")
    print(f"  Marked {non_toronto_count} listings as non-Toronto (empty region)")

    df["region"] = new_region
    return df
...
def main():
    ...
    # Step 2.5: Standardize neighborhoods (MC-320)

    # Map generic "Toronto" / "" / city-level strings to official Toronto

    # neighbourhoods via neighbourhood_lookup.py, so fair_value grouping by

    # (neighborhood, beds) actually has signal.

    print()

    print("=" * 80)

    print("  [2.5] NEIGHBOURHOOD STANDARDIZATION (MC-320)")

    print("=" * 80)

    print()

    df = standardize_neighbourhoods(df)
    ...
    try:
        ...
        # MC-326: notify any subscribed saved searches about *new* matching
        # listings. Runs after email/SMS so users with overlapping subscriptions
        # see the digest a few seconds after the bulk alerts.
        try:
            from persist import check_and_send_saved_search_alerts
            saved_results = check_and_send_saved_search_alerts()
            print(f"  Saved-search alerts: {saved_results['sent']} sent, "
                  f"{saved_results['skipped_no_matches']} no-new-matches, "
                  f"{saved_results['skipped_rate_limit']} rate-limited, "
                  f"{saved_results['errors']} errors "
                  f"(checked {saved_results['checked']} searches)")
        except Exception as e:
            print(f"  Saved-search alerts check skipped: {e}")
