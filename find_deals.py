...
def standardize_neighbourhoods(df):
    ...
    Falls back to title-based extraction when the neighbourhood is generic.
    Adds/updates 'region' column via region_map.neighbourhood_to_region.
    ...
    for idx in range(len(df)):
        ...
        # Generic — try lookup + title fallback
        result = _standardize(raw, title)
        if result is None:
            new_nbhd.append(raw)
